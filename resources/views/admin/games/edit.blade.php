...
<div class="container">
    ...
    <form action="{{ route('admin.games.update', $game) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <input type="date" class="form-control" id="release_date" name="release_date" value="{{old("
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control" id="publisher_link" name="publisher_link" value="{{old("
                publisher_link",$game->publisher_link)}}" placeholder="Insert Publisher link">
        </div>
        <div class="mb-3">
            <label for="publisher" class="form-label">Publisher's Name</label>
            <input type="text" class="form-control" id="publisher" name="publisher" value="{{old("
                publisher",$game->publisher)}}"
            placeholder="Insert Publisher Name">
        </div>
        <div class="mb-3">
            <label for="platform_link" class="form-label">Platform's Link</label>
            <input type="text" class="form-control" id="platform_link" name="platform_link" value="{{old("
                platform_link",$game->platform_link)}}" placeholder="Insert Platform Link">
        </div>
        <div class="mb-3">
            <label for="platform" class="form-label">Platform's Name</label>
            <input type="text" class="form-control" id="platform" name="platform" value="{{old("
                platform",$game->platform)}}"
            placeholder="Insert Platform Name">
        </div>
        <div class="mb-3">
            <label for="description" class="form-label">Game's Description</label>
            <input type="text" class="form-control" id="description" name="description" value="{{old("
        ...
        <div class="form-check form-switch mb-3">
            <input class="form-check-input" type="checkbox" role="switch" id="image-handler"
                @checked(!Str::startsWith($game->thumb, 'http'))>
        ...
        <div class="thumb-input-wrapper">
            ...
            <div class="mb-3 @if(Str::startsWith($game->thumb, 'http')) d-block @else d-none @endif" id="link-file">
                ...
                <input type="text" class="form-control" id="thumb" name="thumb"
                    placeholder="Insert the path of game's image" @if(Str::startsWith($game->thumb, 'http')) value="{{
                old('thumb', $game->thumb) }}"@endif>
            </div>
        </div>
        <div class="form-check form-switch">
            <input class="form-check-input" type="checkbox" role="switch" id="flexSwitchCheckDefault" name="highlight"
                @checked(old("highlight", $game->highlight)) >
            <label class="form-check-label" for="flexSwitchCheckDefault">Highlight this game</label>
        </div>
